...
public class ProjectPutupView extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ProjectPutupView frame = new ProjectPutupView(empId, password);
	 ...
	 * Create the frame.
	 */
	public ProjectPutupView(String empId, String password) {
		...
		JLabel lblNewLabel = new JLabel("프로젝트 등록");
		...
		lblNewLabel_3.setBounds(12, 115, 122, 32);
		contentPane.add(lblNewLabel_3);
		
		budgetBOX = new JTextField();
		budgetBOX.setBounds(153, 118, 261, 28);
		...
		goBOX.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				lblNewLabel_7.setText("날짜는 2023/01/01 형식으로 입력해주세요");
			}
		...
		enrolmentButton = new JButton("등록");
		enrolmentButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String name = nameBOX.getText();		// ������Ʈ��
				String tim = timBOX.getText();		// ����
				String budget = budgetBOX.getText();		//�ѿ���
				String go = goBOX.getText();		// ������
				String end = endBOX.getText();		// ������
				String details = (String)detailsBOX.getSelectedItem();;		//�Һз�
				ProjectPutupVO vo = new ProjectPutupVO(name, tim, budget, go,end,details);
				try {
					dao = new ProjectPutuDAO();
					int cnt = dao.regist(vo);
					System.out.println(cnt + "프로젝트 등록 성공");
					
					JOptionPane.showMessageDialog(null, "프로젝트 등록 성공", "프로젝트 등록 알람 메세지", JOptionPane.PLAIN_MESSAGE);
					...
					JOptionPane.showMessageDialog(null, "프로젝트 등록실패" + e2.getMessage());
				}
